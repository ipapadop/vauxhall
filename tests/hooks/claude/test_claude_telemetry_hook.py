...
from vauxhall.hooks.claude.telemetry_hook import _create_telemetry_client, main
...
def _run(*events: dict) -> MagicMock:
    ...
    with (
        patch(f"{HOOK}._create_telemetry_client") as client_factory,
...
def test_claude_telemetry_connect_timeout_fits_hook_budget() -> None:
    """MQTT connection setup must finish within the hook timeout."""
    assert _create_telemetry_client().client.connect_timeout == 1.0
...
def test_claude_events_publish_dashboard_states(
    ...
    with patch(f"{HOOK}.tempfile.gettempdir", return_value=str(tmp_path)):
...
def test_claude_ignores_events_without_dashboard_state(hook_data: dict) -> None:
    """Events that do not change the agent's state must not publish telemetry."""
    client = _run({**BASE, **hook_data})
...
def test_claude_concurrent_tool_durations_use_tool_use_ids(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(f"{HOOK}.time.time", side_effect=[1000.0, 1001.0, 1003.0, 1007.0]),
    ):
        client = _run(
...
def test_claude_failed_tool_reports_duration(tmp_path: Path) -> None:
    ...
    with (
        patch(f"{HOOK}.tempfile.gettempdir", return_value=str(tmp_path)),
        patch(f"{HOOK}.time.time", side_effect=[1000.0, 1002.5]),
    ):
